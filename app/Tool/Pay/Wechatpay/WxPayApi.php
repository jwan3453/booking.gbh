...
namespace App\Tool\Pay\Wechatpay;

use Illuminate\Support\Facades\Config;
use App\Tool\Pay\Wechatpay\WxPayException;
/**
 *
...
//        }
    ...
    {
        //如果不需要上报数据
        if(config::get('wechatpay.REPORT_LEVENL') == 0){
            return;
        }
        //如果仅失败上报
        if(config::get('wechatpay.REPORT_LEVENL') == 1 &&
    ...
    private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
    {
        ...
        //如果有配置代理这里就设置代理
        if(config::get('wechatpay.CURL_PROXY_HOST') != "0.0.0.0"
            &&config::get('wechatpay.CURL_PROXY_PORT')  != 0){
            curl_setopt($ch,CURLOPT_PROXY,config::get('wechatpay.CURL_PROXY_HOST') );
            curl_setopt($ch,CURLOPT_PROXYPORT, config::get('wechatpay.CURL_PROXY_PORT') );
        }
